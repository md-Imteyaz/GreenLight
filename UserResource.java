...
import com.gl.platform.config.Constants;
...
public class UserResource {
	...
	public UserResource(UserService userService, JhiUserRepository userRepository, MailService mailService) {
	...
	public ResponseEntity<JhiUser> createUser(@Valid @RequestBody JhiUserDTO userDTO) throws URISyntaxException {
		...
		if (userDTO.getId() != null) {
			...
			JhiUser newUser = userService.createUser(userDTO);
	...
	@Timed
	@Secured(AuthoritiesConstants.ADMIN)
	public ResponseEntity<JhiUserDTO> updateUser(@Valid @RequestBody JhiUserDTO userDTO) {
		...
		Optional<JhiUserDTO> updatedUser = userService.updateUser(userDTO);
	...
	@GetMapping("/users")
	@Timed
	public ResponseEntity<List<JhiUserDTO>> getAllUsers(Pageable pageable) {
		final Page<JhiUserDTO> page = userService.getAllManagedUsers(pageable);
	...
	@Timed
	public ResponseEntity<JhiUserDTO> getUser(@PathVariable String login) {
		log.debug("REST request to get User : {}", login);
		return ResponseUtil.wrapOrNotFound(userService.getUserWithAuthoritiesByLogin(login).map(JhiUserDTO::new));
	}
